...
ngx_master_process_cycle(ngx_cycle_t *cycle)
{
    ...
    ngx_set_inherited_listen_sockets(cycle);   //这个将监听套接字加入NGINX_VAR,有点迷惑，别的地方没有找到NGINX_VAR的赋值，可能是参考
    ...
    for ( ;; ) {
        ...
        if (ev == WAIT_OBJECT_0 + 3) {
            ...
            */
            ...
            ngx_set_inherited_listen_sockets(cycle);    //设置为可继承监听套接字

            if (ngx_start_worker_processes(cycle, NGX_PROCESS_RESPAWN) == 0) {  //创建工作进程
                exit(2);
            }
            /*---code --- end*/
            continue;
...
ngx_quit_worker_processes(ngx_cycle_t *cycle, ngx_uint_t old)
{
    ...
    WaitForMultipleObjects(count, handles, TRUE, 2000); //再等待一次，全部退出
    ngx_terminate_worker_processes(cycle);
}
